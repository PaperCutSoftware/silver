...
func TestFileOps(t *testing.T) {
	...
	err = WriteFileString(file.Name(), data, 0640)
	if err != nil {
	...
	if read := ReadStringFromFile(file.Name(), "default"); read != data {
...
func TestReadStringFromFile(t *testing.T) {
	...
	_ = WriteFileString(file.Name(), "Test-1234", 0600)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := ReadStringFromFile(tt.args.file, tt.args.def); got != tt.want {
